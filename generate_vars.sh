...
COMMITTER_NAME=$(echo "${BUILD_INFO}" | tail -n23 | grep committer_name | cut -f2- -d: | cut -d\" -f2) && echo 'export COMMITTER_NAME=$(echo "${BUILD_INFO}" | tail -n23 | grep committer_name | cut -f2- -d: | cut -d\" -f2)' >> $BASH_ENV
