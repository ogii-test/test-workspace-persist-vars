...
COMMITTER_NAME=$(echo $BUILD_INFO | tail -n23 | grep committer_name | cut -f2- -d: | grep -o -P "(?<=\").*(?=\")")' && echo 'export COMMITTER_NAME=$(echo $BUILD_INFO | tail -n23 | grep committer_name | cut -f2- -d: | grep -o -P "(?<=\").*(?=\")")' >> $BASH_ENV
